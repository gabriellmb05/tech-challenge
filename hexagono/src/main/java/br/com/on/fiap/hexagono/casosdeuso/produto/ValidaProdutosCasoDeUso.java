...
public class ValidaProdutosCasoDeUso implements ValidaProdutoPortaEntrada {
    ...
    private Set<Long> buscarProdutosExistentes(List<Long> idsProdutos) {
        return persisteProdutoPortaSaida
                .buscaProdutoPorIdsLista(idsProdutos)
    ...
    private void validarSeTodosProdutosExistem(List<Long> idsProdutos, Set<Long> produtosExistentes) {
        List<Long> idsFaltantes = idsProdutos.stream()
                .filter(id -> !produtosExistentes.contains(id))
                .toList();

        if (!idsFaltantes.isEmpty()) {
            throw new ProdutoNaoEncontradoExcecao(
                    MessageError.MSG_PRODUTOS_NAO_ENCONTRADOS.getMensagem(), idsFaltantes.toArray());
        }
    }
}
