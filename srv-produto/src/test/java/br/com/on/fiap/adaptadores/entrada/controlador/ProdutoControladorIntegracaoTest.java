...
class ProdutoControladorIntegracaoTest {
	...
	@Transactional
	@Order(1)
	@DisplayName("Dado um produto novo, quando não informar o nome do produto, então deve retornar erro de validação 'O atributo nome é obrigatório'")
	void dadoProdutoNovo_quandoInserirProdutoSemInformarNome_entaoDeveRetornarErroDeValidacao() throws Exception {
		mockMvc.perform(post("/produtos").contentType(MediaType.APPLICATION_JSON)
				...
				.andExpect(jsonPath("$.errors['nome']").value("O atributo nome é obrigatório")).andReturn();
	}

	@Test
	@Transactional
	@Order(2)
	@DisplayName("Dado um produto novo, quando nao informar a categoria do produto, então deve retornar erro de validação 'O atributo categoria é obrigatório'")
	void dadoProdutoNovo_quandoInserirProdutoSemInformarCategoria_entaoDeveRetornarErroDeValidacao() throws Exception {
		mockMvc.perform(post("/produtos").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(DataPoolProdutoSolicitacaoDTO.gerarComCategoria(null))))
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.errors['categoria']").value("O atributo categoria é obrigatório")).andReturn();
	}

	@Test
	@Transactional
	@Order(3)
	@DisplayName("Dado um produto novo, quando nao informar o preço do produto, então deve retornar erro de validação 'O atributo preço é obrigatório'")
	void dadoProdutoNovo_quandoInserirProdutoSemPrecoCategoria_entaoDeveRetornarErroDeValidacao() throws Exception {
		mockMvc.perform(post("/produtos").contentType(MediaType.APPLICATION_JSON)
				...
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.errors['preco']").value("O atributo preço é obrigatório")).andReturn();
	}

	@Test
	@Transactional
	@Order(3)
	@DisplayName("Dado um produto novo, quando informar uma categoria inválida, então deve retornar erro de validação 'Categoria (categoria) não encontrada'")
	void dadoProdutoNovo_quandoInserirProdutoComCategoriaInvalida_entaoDeveRetornarErroDeValidacao() throws Exception {
		mockMvc.perform(post("/produtos").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(DataPoolProdutoSolicitacaoDTO
						.gerarNomeCategoriaPreco("Coca-cola", "inválida", BigDecimal.TEN))))
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.detail").value("Categoria (inválida) não encontrada")).andReturn();
	}
	...
	void dadoProdutoExistente_quandoAtualizarProdutoComIdInvalido_entaoDeveRetornarErro() throws Exception {
		mockMvc.perform(put("/produtos/{id}", 5).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(
						DataPoolProdutoSolicitacaoDTO.gerarNomeCategoriaPreco("Coca-cola", "BEBIDA", BigDecimal.TEN))))
				.andExpect(status().isNotFound()).andExpect(jsonPath("$.detail").value("Produto (5) não encontrado"))
				.andReturn();
	}
	...
	@Transactional
	@Order(6)
	@DisplayName("Dado um produto existente, quando atualizar o produto sem informar o nome do produto, então deve retornar erro de validação 'O atributo nome é obrigatório'")
	void dadoProdutoExistente_quandoAtualizarProdutoSemInformarNome_entaoDeveRetornarErroDeValidacao()
			throws Exception {
		mockMvc.perform(put("/produtos/{id}", 1).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(DataPoolProdutoSolicitacaoDTO.gerarComNome(null))))
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.errors['nome']").value("O atributo nome é obrigatório")).andReturn();
	}
	...
	@Transactional
	@Order(7)
	@DisplayName("Dado um produto existente, quando atualizar o produto sem informar o categoria do produto, então deve retornar erro de validação 'Categoria (categoria) não encontrada'")
	...
	@Transactional
	@Order(8)
	@DisplayName("Dado um produto existente, quando atualizar o produto sem informar o preço do produto, então deve retornar erro de validação 'O atributo preço é obrigatório'")
	...
	@Transactional
	@Order(10)
	@DisplayName("Dado um produto existente, quando deletar o produto informando id inválido, então deve retornar erro de validação 'Produto (id) não encontrado'")
	...
	void dadoProdutoExistente_quandoConsultarProdutoInformandoIdInvalido_entaoDeveRetornarErroDeValidacao()
		...
		mockMvc.perform(get("/produtos/{id}", 8).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isNotFound()).andExpect(jsonPath("$.detail").value("Produto (8) não encontrado"))
				.andReturn();
	}
	...
	void dadoProdutoExistente_quandoConsultarProduto_entaoDeveRetornarProduto() throws Exception {
		mockMvc.perform(get("/produtos/{id}", 1).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
	...
	@Transactional
	@Order(15)
	@DisplayName("Dado produtos existentes, quando listar produtos com filtro de categoria, então deve retornar produtos filtrados")
	...
	@Transactional
	@Order(16)
	@DisplayName("Dado produtos existentes, quando listar produtos com filtro de nome, então deve retornar produtos filtrados")
	...
	@Transactional
	@Order(18)
	@DisplayName("Dado produtos existentes, quando listar produtos com múltiplos filtros, então deve retornar produtos filtrados")
